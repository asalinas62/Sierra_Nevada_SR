...
#              print("sensor "+str(sensor)+" modo 1 ", j, pm1)
                pm1=maxrela(bm1)
#              print(pm1)
            bm2= rs_LO[j,fre1p:fre2p]
            pm2=np.argmax(bm2)
            if pm2 == 0 or pm2 == len(bm2)-1 :
#                print("sensor "+str(sensor)+" modo 2 ", j, pm2+fre1p)
                pm2=maxrela(bm2)
#                print(pm2+fre1p)
            pm2=pm2+fre1p
            bm3= rs_LO[j,fre2p:]
...
#                print("sensor "+str(sensor)+" modo 3 ", j, pm3+fre2p)
                pm3=maxrela(bm3)
#                print(pm3+fre2p)
            pm3=pm3+fre2p
            #pr[j]=np.array([rs_LO[j,pm1],(freajus[pm1]+freajus[pm1-1])/2*df*(rs_LO[j,pm1]-rs_LO[j,pm1-1])/(rs_LO[j,pm1]+rs_LO[j,pm1-1]),\
            #                rs_LO[j,pm2],freajus[pm2],\
            #                rs_LO[j,pm3],freajus[pm3]])
